...
async function run() {
  ...
  const uniqueKeys = [...new Set(libData.items.map(i => i.categoryKey))];
  console.log("Unique category keys in library:", uniqueKeys);
  const kitchenApplianceItem = libData.items.find(item => item.categoryKey.includes('Appliances'));
  ...
  console.log(`Initial/Base Budget: $${finalProject.initialBudget}`);
  console.log(`Current Budget (updated): $${finalProject.currentBudget}`);
  if (finalProject.currentBudget !== finalProject.initialBudget + 1000) {
    throw new Error(`Budget update incorrect. Expected ${finalProject.initialBudget + 1000}, got ${finalProject.currentBudget}`);
  }
  ...
  const pdfPath = path.join(__dirname, '../server/uploads/pdfs', pdfFileName);
  try {
